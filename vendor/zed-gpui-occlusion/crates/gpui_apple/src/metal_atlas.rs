...
impl MetalAtlas {
    ...
    /// Drops every cached glyph and image texture. The next paint repopulates
    /// the atlas from GPUI's CPU-side scene and image data.
...
mod tests {
    ...
    #[test]
    fn test_clear_releases_textures_and_stale_tiles() {
        let Some(atlas) = create_atlas() else {
            return;
        };
        ...
        let _texture = atlas.metal_texture(tile.texture_id);
        atlas.clear();

        let lock = atlas.0.lock();
        assert!(lock.tiles_by_key.is_empty());
        assert!(lock.monochrome_textures.textures.is_empty());
        assert!(lock.monochrome_textures.free_list.is_empty());
        assert!(lock.polychrome_textures.textures.is_empty());
        assert!(lock.polychrome_textures.free_list.is_empty());
        drop(lock);

        let rebuilt_tile = insert_tile(
            &atlas,
            &key,
            Size {
                width: DevicePixels(64),
        ...
        let _rebuilt_texture = atlas.metal_texture(rebuilt_tile.texture_id);
    }
}
