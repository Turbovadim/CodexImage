...
impl MetalRenderer {
    ...
    pub fn new(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>, transparent: bool) -> Self {
        ...
        let layer = Self::create_layer(&device, opaque, 1.0, false);

        Self::new_internal(device, Some(layer), opaque, instance_buffer_pool)
    }
    ...
    fn create_layer(
        ...
        layer.set_contents_scale(contents_scale);
    ...
    fn new_internal(
        ...
        Self {
            device,
            windowed: layer.is_some(),
            contents_scale: layer.as_ref().map_or(1.0, |layer| layer.contents_scale()),
            layer,
    ...
    pub fn update_contents_scale(&mut self, contents_scale: f64) {
    ...
    pub fn update_drawable_size(&mut self, size: Size<DevicePixels>) {
        ...
        if self.gpu_resources_released {
            return;
        }
        self.apply_drawable_size(size);
    }

    fn apply_drawable_size(&mut self, size: Size<DevicePixels>) {
        self.set_layer_drawable_size(size);
        self.update_path_intermediate_textures(size);
    }

    fn set_layer_drawable_size(&self, size: Size<DevicePixels>) {
        if let Some(layer) = &self.layer {
            let ns_size = NSSize {
    ...
    /// Releases resources whose contents can be recreated by a full paint.
    /// Pipeline state and the Metal device stay warm for a fast restore.
    pub fn release_gpu_resources(&mut self) -> bool {
        if self.gpu_resources_released {
            return false;
        }

        self.gpu_resources_released = true;
        if let Some(layer) = self.layer.take() {
            self.contents_scale = layer.contents_scale();
            drop(layer);
        }
        ...
        #[cfg(any(test, feature = "bench-support", feature = "test-support"))]
        {
            self.headless_render_target = None;
        }
        true
    }

    /// Recreates window-sized targets. Atlas textures are rebuilt lazily by
    /// the forced frame requested by the platform window.
    pub fn restore_gpu_resources(&mut self) -> bool {
        if !self.gpu_resources_released {
            return false;
        }

        if self.windowed && self.layer.is_none() {
        ...
        self.gpu_resources_released = false;
        self.apply_drawable_size(self.drawable_size);
        true
    }

    pub fn gpu_resources_released(&self) -> bool {
        self.gpu_resources_released
    }
...
mod tests {
    ...
    #[test]
    fn gpu_resources_restore_at_the_latest_hidden_size() {
        ...
        layer.set_device(&device);
        let mut renderer =
            MetalRenderer::new_internal(device, Some(layer), true, instance_buffer_pool);
        renderer.update_drawable_size(size(DevicePixels(64), DevicePixels(48)));

        assert!(renderer.path_intermediate_texture.is_some());
        assert!(renderer.release_gpu_resources());
        assert!(!renderer.release_gpu_resources());
        assert!(renderer.gpu_resources_released());
        assert!(renderer.path_intermediate_texture.is_none());
        assert!(renderer.path_intermediate_msaa_texture.is_none());
        assert!(renderer.layer().is_none());

        let hidden_size = size(DevicePixels(96), DevicePixels(72));
        renderer.update_drawable_size(hidden_size);
        renderer.update_contents_scale(2.0);
        assert_eq!(renderer.drawable_size, hidden_size);
        assert!(renderer.path_intermediate_texture.is_none());

        assert!(renderer.restore_gpu_resources());
        assert!(!renderer.restore_gpu_resources());
        assert!(!renderer.gpu_resources_released());
        let restored_layer = renderer.layer().expect("restore should recreate the layer");
        assert_eq!(restored_layer.drawable_size().width, 96.0);
        assert_eq!(restored_layer.drawable_size().height, 72.0);
        assert_eq!(restored_layer.contents_scale(), 2.0);
        let path_texture = renderer
            .path_intermediate_texture
            .as_ref()
            .expect("restore should recreate the path target");
        assert_eq!(path_texture.width(), 96);
        assert_eq!(path_texture.height(), 72);
    }
}
